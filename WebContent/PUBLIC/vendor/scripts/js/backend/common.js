...
function focusFirst(field_name, field_value, id)
{
	if(id == 0)
	{
		if($('[name="'+field_name+'"]'))
		{
			if($('[name="'+field_name+'"]')[0]){ $('[name="'+field_name+'"]')[0].focus(); }
...
function openFirstErrTab(key_arr, tab_id, content_id_prefix)
{
	...
	var sel = null;
	var getTab = false;
	//alert($('#'+tab_id).children('ul:first').children(':nth-child(2)').html());
	//$('#'+tab_id).children('ul:first').children(':nth-child(2)').children(':first').children(':first').trigger('click');
	//alert($('#'+tab_id).children('div:first').children('div:eq(1)').html());
	$.each($('#'+tab_id).children('div:first').children('div'), function(index, obj){
			for(var j= 0; j < key_arr.length; j++)
			{
				err_id = '[name="'+key_arr[j].key+'_err"]';
	...
	/*for(var i = 1; i <= $('#'+tab_id).children('ul:first').children().length ; i++)
	{
		for(var j= 0; j < key_arr.length; j++)
		{
			err_id = '[name="'+key_arr[j].key+'_err"]';
	...
	if($(floatingbar))
	{
		$(window).scroll(function() {
			...
			if($(top_button).position())
			{
				...
				{
					$(floatingbar).animate({ height: 0}, 50);
				}
			}
		});
	}
...
function commonFilePath(file_path, filename)
{
		...
		switch(ext.toLowerCase())
		{
			...
			case 'FLV':
			case 'SWF':
				img_path = 'data/adminFiles/extImages/flash.png';
				break;
			case 'mp3':
			case 'MP3':
				img_path = 'data/adminFiles/extImages/mp3.png';
				break;
			case 'avi':
			case 'wmv':
			case 'wma':
			...
			case 'AVI':
			case 'WMV':
				img_path = 'data/adminFiles/extImages/avi_thumb.png';
				break;
			case 'pdf':
			case 'doc':
			case 'docx':
			...
			case 'PPT':
			case 'PPTX':
				img_path = 'data/adminFiles/extImages/'+ext.toLowerCase()+'.png';
				break;
			case 'jpg':
			case 'png':
			case 'gif':
			...
			case 'ico':
				...
				break;
			default:
				img_path = 'data/adminFiles/extImages/all_file_thumb.png';
				break;
		}
	return img_path;
}
...
function commonRemoveTabs(settingObj)
{
	if(settingObj.form_action.dynamic_field_generator.has_tab)
	{
		$( "#" + settingObj.form_action.dynamic_field_generator.tab_id ).find( "ul:first>span>a[rel='virtual']" ).parent('span:first').remove();
...
function commonGetFormData(form_id, hasTinyMCE)
{
	...
	$.each($('#'+form_id).serializeArray(), function(i, field) {
		...
		fieldObj[filename] = ( hasTinyMCE === true && tinymce.EditorManager.get($('[name="'+filename+'"]').attr('id')) && !tinymce.EditorManager.get($('[name="'+filename+'"]').attr('id')).isHidden() ) ? tinymce.EditorManager.get($('[name="'+filename+'"]').attr('id')).getContent() : filevalue;
	});
	return fieldObj;
}
...
function commonResetFormFields(settingObj)
{
		...
		$.each(settingObj.form_action.extra_field_for_reset,function(index, obj){
			switch(obj.field_type)
			{
				case 'html':
						...
						$('[name="'+obj.field_id+'"]').html(obj.field_value);
					break;
				case 'val':
	...
	var upload_field = $("#"+settingObj.form_action.form_id ).find('input[type="file"]');
	if(upload_field.length > 0)
	{
		$.each(upload_field, function(key, field){
			var upload_field_id = $(field).attr('name');
			$("input[name='"+upload_field_id+"']").data("kendoUpload").enable();
		});
	}
...
function commonAddFormError(key,errMsg)
{
	...
	{
		var msg_field = (settingObj && settingObj.form_action && settingObj.form_action.common_msg_field_id) ? settingObj.form_action.common_msg_field_id : 'actionMessage';
		$('#'+msg_field).append(errMsgDesign(field_name+ ' : '+errMsg));
	}
	else
	{
		$('#'+field_name+'-input').addClass('ui-state-error');
		$('[name="'+field_name+'"]').addClass('ui-state-error');
		$('[name="'+field_name+'[]"]').addClass('ui-state-error');
		$('#'+err_id).append(errFieldMsgDesign(errMsg));
		$('[name="'+err_id+'"]').append(errFieldMsgDesign(errMsg));
	}
}
	...
	var grid = $("#"+settingObj.grid_id).kendoGrid({
				  ...
				  dataSource: {
					  ...
					  error: function (e) {
						   ...
						   commonMsgDialog(settingObj, errMsgDesign(e.xhr.responseText));
					  },
					  transport: {
						  read: {
							  type: "POST",
							  ...
							  complete: function(e) {
								  ...
								  {
									 commonMsgDialog(settingObj, errMsgDesign(json.msg));
								  }
							  }
						  },
						...
						try
						{
							...
							if(json_arr.status == 'ok')
							{
								...
								$.each($('input.check_btn:checked'), function(index, obj){
										...
										$(obj).parents('tr:first').find('a.publish_btn').attr('title', publish_icon_title);
										if((settingObj.common_action.publish_active_icon_link))
										{
							 ...
							 success: function(response)
							 {
								 try
								 {
									...
									if(json_arr.status == 'ok')
									{
										...
										{
											$(self).html(obj.multi_action_dest_data_paction[result_value].icon_link_class).attr('rel',arr[0]+"_"+arr[1]+"_"+obj.multi_action_dest_data_paction[result_value].paction);
											$(self).attr('title', obj.multi_action_dest_data_paction[result_value].title);
										}
										if(obj.success_func)
										{
		...
		$(listObj.list_id).change(function(){
				...
				if($(self).val() == 'any' || $(self).val() == '')
				{
					$.each(listObj.sub_list.list_loader_info.on_loading, function(key, obj){
							$(obj.id).html('<option value="">'+obj.normal_lang+'</option>');
						});
				}
				else
				{
					$.ajax({
						  ...
						  beforeSend: function(){
						  			$.each(listObj.sub_list.list_loader_info.on_loading, function(key, obj){
										$(obj.id).html('<option value="">'+obj.loading_lang+'</option>');
									});
						  ...
						  success: function(response){
							//alert(response);
							var json_arr = eval("("+response+")");
							var option = '<option value="">'+listObj.sub_list.list_any_value+'</option>';
							if(json_arr.status == 'ok')
							{
							...
							{
								$(listObj.sub_list.list_id).html('<option value="">'+json_arr.msg+'</option>');
							}
						  ...
						  error: function(xhr, status, error){
							  ...
							  commonMsgDialog(msgSettingObj, errMsgDesign(msg));
						  }
					});
				}
			});
...
function kendoDateTimeCalendar(element, settingObj)
{
    ...
    if (settingObj.calendar)
    {
        options = $.extend({}, options, settingObj.calendar);
        options.format = settingObj.calendar.dateTimeFormat;
    }

    element.kendoDateTimePicker(options);
}
...
function kendoDateCalendar(element, settingObj)
{
    ...
    {
        options = $.extend({}, options, settingObj.calendar);
        options.format = settingObj.calendar.dateFormat;
    }
...
function kendoSelectBox(settingObj)
{
	if(settingObj && settingObj.selectbox)
	{
		$.each(settingObj.selectbox, function(index, obj){
			$(obj.combobox_id).kendoComboBox(obj.combobox_settings);
		});
	}
}
