...
{
	$("input[name='"+upload_field+"']").kendoUpload({
			multiple: multi,
			showFileList: true,
			localization:{
			...
			cancel: window.onCancel || null,
			complete: window.onComplete || null,
			error: function(e, index) {
				var err_msg = (e.XMLHttpRequest.responseText) ? e.XMLHttpRequest.responseText : '';
				switch(e.operation )
				{
					case 'upload':
						var files = e.files;
						$.each(files, function() {
							commonMsgDialog(settingObj,errMsgDesign(language_array['portfolio_file_upload_err'] + this.name + '. ' + err_msg) );
						});
						break;
					case 'remove':
			...
			progress: window.onProgress || null,
			remove: function onRemove(e) {
				...
				//alert(e.files[0].name + '  '+upload_value_field);
				$.each( $('div.upload'),  function(index, obj){
					if($(obj).children('img').attr('title') ==  e.files[0].name)
					{
						$(obj).remove();
					}
				});
			...
			select: function(e) {
						...
						//console.log(setting_info["file_type"]);
						...
						$.each(files, function() {
							...
							if(ext_arr[1])
							{
								if (($.inArray(ext_arr[1].toLowerCase(), permitted_ext_arr) == -1) && ($.inArray(ext_arr[1].toUpperCase(), permitted_ext_arr) == -1)) {
									...
									commonMsgDialog(settingObj, errMsgDesign(language_array['portfolio_file_extension_dy_not_supportable_ext1']+' " '+ext_arr[1]+' " '+language_array['portfolio_file_extension_dy_not_supportable_ext2']+ '  ' +language_array['portfolio_file_extension_dy_support']+'" '+support_file_extension+' "'));
									e.preventDefault();
								}
								else if((file_size > permitted_size) && (permitted_size > 0))
								{
									commonMsgDialog(settingObj, errMsgDesign(language_array['portfolio_file_size_dy_support']+setting_info[setting_info['file_max_size_field']]));
									e.preventDefault();
								}
							}
							else
							{
								commonMsgDialog(settingObj, errMsgDesign(language_array['portfolio_file_no_ext_dy_support']));
								e.preventDefault();
							}
						});
					},
			success: function onSuccess(e) {
				...
				if (e.operation == "upload")
				{
				   ...
				   else if(data.status == 'errV')
				   {
					   var msg = '';
					   $.each(data.msg, function(i, obj) {
						   msg += obj.key+' : '+ obj.value+"<br />";
					   });
					   commonMsgDialog(settingObj, errMsgDesign(msg));
				   }
				   else
				   {
					   commonMsgDialog(settingObj, errMsgDesign(data.msg));
				   }
				}
			...
			upload: function(e) {
						...
						e.data['security'] = $('input[name="security"]').val();
					}
		});
}
...
{
	var ext = fileObj.extension.toLowerCase();
	if(ext == '.jpg' || ext == '.png' ||  ext == '.bmp'  || ext == '.gif' || ext == '.ico')
	{
		return true;
	}
	else
...
{
	var block = 'selected_file_'+ $("#"+upload_value_field).attr('name');
	$('[name="'+block+'"]').children('div.box').each(function(index, element) {
...
function deleteFiles(main_title, related_title, delete_url, common_delete_title)
{
	...
	$('input.radio_style').bind('click', function(e){
		var self = this;
		$(self).parents('div:first').parents('span:first').children('div').each( function(){
			$(this).children('span:first').text(related_title);
			$(this).children('span:first').removeClass('label label-success');
			$(this).children('span:first').addClass('label label-warning');
		});
		$(self).parents('div:first').children('span:first').text(main_title);
		$(self).parents('div:first').children('span:first').removeClass('label label-warning');
		$(self).parents('div:first').children('span:first').addClass('label label-success');
	});

	$('a.delete_image_lnk').unbind('click');
	$('a.delete_image_lnk').bind('click', function(e){
			var self = this;
			var file_name = (settingObj && settingObj.uploader_settings && settingObj.uploader_settings.upload_manager.include_file_path) ? $(self).attr('rel') : $(self).attr('file_name');
			var content_id = $(self).attr('field-name');
			//var put_file_arr = content_id.substr('selected_file_');
			var put_file_obj = '[name="'+content_id+'"]';
	...
	$('a.delete_uploaded_image_link').bind('click', function(e){
			...
			var delete_file_name = (settingObj && settingObj.uploader_settings && settingObj.uploader_settings.upload_user.include_file_path) ? $(self).attr('file_path')+'/'+file_name : file_name;

			var file_content = $('#'+field_name).val();
			common_delete_title = $(self).html();
			...
			$.ajax({
				...
				success: function(response){
					...
					if(json_arr.status == 'ok')
					{
						...
						for(var i = 0; i < file_arr.length; i++)
						{
							if(file_arr[i] != delete_file_name)
							{
								put_file = (put_file == '')? file_arr[i] : put_file +','+ file_arr[i];
							}
						}
						...
						{
							var upload_field_id = 	parent_ul.parent("div:first").find('input[type="file"]:first').attr('name');
